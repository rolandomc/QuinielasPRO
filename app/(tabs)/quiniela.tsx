...
export default function QuinielaScreen() {
  ...
  if (loading) return <View style={[styles.center,{paddingTop:insets.top}]}><ActivityIndicator color={C.accent} size="large"/></View>;

  const yaGuardo=!!quiniela, esPagado=quiniela?.estado_pago==='pagado';
          ...
          <>
            ...
            {!yaGuardo && (
              <View style={styles.progressBox}>
                ...
                <View style={styles.progressBar}><View style={[styles.progressFill,{width:`${selCount/partidos.length*100}%` as any}]}/></View>
              </View>
            )}
            {partidos.map(p => (
              <View key={p.id} style={styles.partidoCard}>
                ...
                <View style={styles.opcionesRow}>
                  {(['1','X','2'] as const).map(op => {
                    ...
                    return (
                      <TouchableOpacity key={op} style={[styles.opcion,activo&&styles.opcionActiva,yaGuardo&&{opacity:0.7}]} onPress={()=>seleccionar(p.id,op)} disabled={yaGuardo} activeOpacity={0.7}>
                        <Text style={[styles.opcionLetra,activo&&styles.opcionLetraActiva]}>{op}</Text>
                        <Text style={[styles.opcionEquipo,activo&&styles.opcionEquipoActivo]} numberOfLines={1}>{op==='1'?p.local.slice(0,7):op==='X'?'Empate':p.visitante.slice(0,7)}</Text>
                      </TouchableOpacity>
                    );
                  })}
                </View>
...
const styles = StyleSheet.create({
  ...
  emptyBox:{alignItems:'center',padding:60}, emptyEmoji:{fontSize:60,marginBottom:16},
  ...
  partidoFecha:{color:C.accent,fontSize:12,fontWeight:'600',marginBottom:10,textAlign:'center',letterSpacing:0.5},
