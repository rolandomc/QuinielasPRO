...
function SelectorQuinielas({
  ...
  jornadas: Jornada[];
  seleccionada: Jornada | null;
  onSeleccionar: (j: Jornada) => void;
  quinielasUsuario: QuinielaDB[];
}) {
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: 8, paddingBottom: 2 }}>
        {jornadas.map(j => {
          ...
          return (
            <TouchableOpacity
              key={j.id}
              style={[selectorStyles.chip, activa && selectorStyles.chipActivo]}
            ...
            >
              ...
              <Text style={[selectorStyles.chipTexto, activa && selectorStyles.chipTextoActivo]} numberOfLines={1}>
                {j.nombre}
              </Text>
...
export default function QuinielaScreen() {
  ...
  // ─ Lista de todas las jornadas abiertas
  ...
  // Todas las quinielas del usuario (para el selector)
  ...
  // Carga TODAS las jornadas abiertas y quinielas del usuario
  const cargarJornadas = useCallback(async () => {
    if (!user) return;
    const { data: jData } = await supabase
      .from('jornadas').select('id,nombre,estado,precio')
  ...
  // Carga partidos y predicciones de la jornada seleccionada
  ...
  const cargar = useCallback(async () => {
    ...
    // Si hay jornada seleccionada, actualizar sus datos; si no, seleccionar la primera
    if (jornada) {
      // refrescar la jornada activa desde la lista nueva
      const jActualizada = (lista || []).find(j => j.id === jornada.id);
      if (jActualizada) {
        setJornada(jActualizada);
        await cargarJornada(jActualizada);
      } else if ((lista || []).length > 0) {
        setJornada((lista || [])[0]);
        await cargarJornada((lista || [])[0]);
      } else {
  ...
  const seleccionarJornada = useCallback(async (j: Jornada) => {
    setJornada(j);
    setPartidos([]); setPredicciones({}); setQuiniela(null);
    ...
    <View style={styles.root}>
      ...
      >
        ...
        <View style={[styles.header, { paddingTop: insets.top + 16 }]}>
          ...
          {/* Precio prominente — se muestra si la jornada tiene precio */}
        ...
        {/* ── SELECTOR (solo si hay más de 1 jornada abierta) ── */}
        <SelectorQuinielas
