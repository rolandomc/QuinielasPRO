 ...
 * Usa crearSolicitudRetiro de lib/retiros.ts (con fallback si la RPC no existe).
 */
...
import { crearSolicitudRetiro } from '../lib/retiros';
...
export default function SolicitarRetiroScreen() {
  ...
  const enviar = async () => {
    if (!formValido || !usuario?.id) return;
    setEnviando(true);
    try {
      await crearSolicitudRetiro({
        ...
        banco,
        clabe: modoDest === 'clabe' ? clabe.replace(/\s/g, '') : null,
        numero_tarjeta: modoDest === 'tarjeta' ? tarjeta.replace(/\s/g, '') : null,
      });
      setExito(true);
      await cargar();
    } catch (e: any) {
      ...
      <View style={[s.root, { paddingTop: insets.top }]}>
        ...
        >
          <View style={s.saldoCard}>
            ...
            {saldo <= 0 && <Text style={s.saldoCero}>No tienes saldo disponible para retirar.</Text>}
          </View>
          ...
          {!pendiente && saldo > 0 && (
            <>
              ...
              {montoNum > saldo && <Text style={s.errorTexto}>El monto supera tu saldo disponible.</Text>}

              <Text style={s.label}>Nombre del titular de la cuenta</Text>
              <TextInput
                style={s.input} value={titular} onChangeText={setTitular}
                placeholder="Como aparece en tu tarjeta/cuenta" placeholderTextColor={C.textMuted}
                ...
                {(['clabe', 'tarjeta'] as const).map(m => (
                  ...
                  >
                    <Ionicons
                      ...
                      size={15}
                      color={modoDest === m ? C.accent : C.textSub}
                    />
                    <Text style={[s.destinoTabTexto, modoDest === m && { color: C.accent }]}>
              ...
              {modoDest === 'clabe' ? (
                <>
                  <TextInput
                    ...
                    placeholder="18 dígitos" placeholderTextColor={C.textMuted}
                ...
                <>
                  <TextInput
                    ...
                    placeholder="16 dígitos" placeholderTextColor={C.textMuted}
                ...
                <View style={s.resumenCard}>
                  ...
                  <View style={s.resumenRow}>
                    ...
                    <Text style={s.resumenVal}>
                      {modoDest === 'clabe' ? `****${clabe.slice(-4)}` : `****${tarjeta.slice(-4)}`}
                    </Text>
                  </View>
                </View>
              )}
